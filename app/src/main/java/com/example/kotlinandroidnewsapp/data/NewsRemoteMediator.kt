...
class NewsRemoteMediator @Inject constructor(
        ...
        try {
            ...
            articleDb.withTransaction {
                ...
                val keys = articles.map {
                    RemoteKeys(articleId = it.index, prevKey = prevKey, nextKey = nextKey)
                }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, Article>) : RemoteKeys? {
        ...
        return state.pages.lastOrNull() { it.data.isNotEmpty()}?.data?.lastOrNull()
            ?.let { article ->
                ...
                articleDb.getRemoteKeysDao().remoteKeysArticlesId(article.index)
            }
    }
    ...
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, Article>): RemoteKeys? {
        ...
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()
            ?.let { article ->
                ...
                Log.d("Response: ", articleDb.getRemoteKeysDao().remoteKeysArticlesId(article.index).toString() )
                articleDb.getRemoteKeysDao().remoteKeysArticlesId(article.index)
            }
    }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.index?.let { article ->
